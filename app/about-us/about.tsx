    ...
    <main className={`overflow-hidden bg-white text-[#191922] ${isRtl ? 'about-rtl text-right' : ''}`} dir={isRtl ? 'rtl' : 'ltr'}>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-16 lg:grid-cols-[1.05fr_.95fr]">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-[#bf2227]/20 bg-white px-4 py-2 text-xs font-bold uppercase tracking-[.2em] text-[#bf2227]">
              <Sparkles size={15} /> {t('About Decasoft')}
            </div>
            ...
            <div className="mt-9 flex flex-wrap gap-4">
              <Link href={href('/contact-us')} className="inline-flex items-center gap-2 rounded-full bg-[#bf2227] px-7 py-3.5 text-sm font-extrabold text-white shadow-lg shadow-red-900/15 transition hover:-translate-y-0.5 hover:bg-[#a91d22]">
                {t('Work with us')} <ArrowRight size={17} />
              </Link>
              <Link href={href('/services')} className="rounded-full border border-gray-300 bg-white px-7 py-3.5 text-sm font-extrabold transition hover:border-[#bf2227] hover:text-[#bf2227]">
            ...
            <div className="mt-10 flex flex-wrap gap-x-7 gap-y-3 text-sm font-semibold text-gray-600">
              {['Customer focused', 'Quality driven', 'Built for long-term growth'].map((item) => (
                <span key={item} className="flex items-center gap-2"><BadgeCheck size={18} className="text-[#bf2227]" />{t(item)}</span>
              ))}
            </div>
          </div>

          <div className="relative mx-auto w-full max-w-xl">
            <div className={`absolute -top-5 h-full w-full rounded-[2rem] border-2 border-[#bf2227]/20 ${isRtl ? '-left-5' : '-right-5'}`} />
            <div className="relative aspect-[4/3] overflow-hidden rounded-[2rem] bg-[#202027] shadow-2xl">
              ...
              <div className="absolute inset-x-0 bottom-0 grid grid-cols-3 divide-x divide-white/20 p-6 text-white sm:p-8">
                <div className="text-center"><p className="text-2xl font-black sm:text-3xl">600+</p><p className="mt-1 text-[10px] uppercase tracking-wider text-white/65 sm:text-xs">{t('Projects')}</p></div>
                <div className="text-center"><p className="text-2xl font-black sm:text-3xl">450+</p><p className="mt-1 text-[10px] uppercase tracking-wider text-white/65 sm:text-xs">{t('Clients')}</p></div>
                <div className="text-center"><p className="text-2xl font-black sm:text-3xl">7+</p><p className="mt-1 text-[10px] uppercase tracking-wider text-white/65 sm:text-xs">{t('Years')}</p></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="px-5 py-20 sm:px-8 lg:py-28">
        <div className="mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-2">
          ...
          <div className="lg:ps-8">
            <p className="text-sm font-bold uppercase tracking-[.2em] text-[#bf2227]">{t('Our story')}</p>
            <h2 className="mt-3 font-montserrat text-3xl font-black leading-tight sm:text-4xl">{t('Built to make digital work feel more connected.')}</h2>
            <p className="mt-6 leading-8 text-gray-600">
              {t('Decasoft began with a straightforward belief: businesses should not have to coordinate disconnected vendors to create one consistent customer experience. We built a multidisciplinary team where designers, developers, marketers, and content specialists solve problems together.')}
            </p>
            <p className="mt-4 leading-8 text-gray-600">
              {t('Today, we partner with growing companies and established organizations to move from an early idea to a polished launch—and continue improving long after delivery.')}
            </p>
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {['One connected delivery team', 'Clear scopes and milestones', 'Senior specialist oversight', 'Support beyond launch'].map((item) => (
                <div key={item} className="flex items-center gap-3 text-sm font-bold"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-red-50 text-[#bf2227]"><Check size={15} /></span>{t(item)}</div>
              ))}
            </div>
          </div>
        </div>
      ...
      <section className="bg-[#1d1c24] px-5 py-20 text-white sm:px-8 lg:py-24">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-8 lg:grid-cols-[.75fr_1.25fr] lg:items-end">
            <div>
              <p className="text-sm font-bold uppercase tracking-[.2em] text-red-300">{t('What guides us')}</p>
              <h2 className="mt-3 font-montserrat text-3xl font-black leading-tight sm:text-4xl">{t('Principles that shape every partnership.')}</h2>
            </div>
            <p className="max-w-2xl leading-7 text-white/60 lg:justify-self-end">{t('Our values are practical standards for how we make decisions, communicate, and deliver.')}</p>
          </div>
          <div className="mt-12 grid gap-px overflow-hidden rounded-[2rem] bg-white/10 sm:grid-cols-2 lg:grid-cols-4">
            {values.map(({ icon: Icon, title, description }, index) => (
              <article key={title} className="bg-[#25242d] p-7 sm:p-8">
                ...
                <h3 className="mt-7 text-xl font-extrabold">{t(title)}</h3>
      ...
      <section className="relative isolate overflow-hidden px-5 py-20 text-white sm:px-8 lg:py-24">
        ...
        <div className="mx-auto max-w-7xl">
          <div className="mx-auto max-w-3xl text-center">
            <p className="text-sm font-bold uppercase tracking-[.2em] text-red-300">{t('What we build')}</p>
            <h2 className="mt-3 font-montserrat text-3xl font-black leading-tight sm:text-4xl lg:text-5xl">{t('Solutions Decasoft Offers')}</h2>
            <p className="mx-auto mt-5 max-w-2xl leading-7 text-white/65">{t('Flexible digital products and business systems shaped around your operations, customers, and growth plans.')}</p>
          </div>
          <div className="mt-12 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {solutions.map((solution) => (
              <div key={solution} className="group flex min-h-16 items-center justify-center rounded-xl border border-white/30 bg-white/[.04] px-4 py-4 text-center backdrop-blur-[2px] transition duration-300 hover:-translate-y-0.5 hover:border-[#e95459] hover:bg-[#bf2227]">
                <span className="text-sm font-extrabold text-white/90 group-hover:text-white">{t(solution)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {industries.slice(0, 8).map(({ name, icon: Icon, image }) => (
              <article key={name} className="group relative aspect-[1.6/1] overflow-hidden rounded-2xl bg-[#1d1c24] shadow-sm">
                <Image src={image} alt={t(name)} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" className="object-cover transition duration-500 group-hover:scale-105" />
          ...
          <div className="mx-auto mt-4 grid max-w-none gap-4 sm:max-w-[calc(50%+0.5rem)] sm:grid-cols-2">
            {industries.slice(8).map(({ name, icon: Icon, image }) => (
              <article key={name} className="group relative aspect-[1.6/1] overflow-hidden rounded-2xl bg-[#1d1c24] shadow-sm">
                <Image src={image} alt={t(name)} fill sizes="(max-width: 640px) 100vw, 25vw" className="object-cover transition duration-500 group-hover:scale-105" />
                <div className="absolute inset-0 bg-black/55 transition duration-300 group-hover:bg-[#bf2227]/75" />
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-white">
                  <Icon size={31} strokeWidth={1.7} />
                  <h3 className="text-base font-extrabold">{t(name)}</h3>
      ...
      <section className="bg-[#f8f5f3] px-5 py-20 sm:px-8 lg:py-24">
        <div className="mx-auto grid max-w-7xl items-center gap-10 lg:grid-cols-[1fr_.9fr]">
          <div>
            ...
            <p className="mt-7 text-sm font-bold uppercase tracking-[.2em] text-[#bf2227]">{t('How we partner')}</p>
            <h2 className="mt-3 max-w-2xl font-montserrat text-3xl font-black leading-tight sm:text-4xl">{t('Close enough to understand. Experienced enough to challenge.')}</h2>
          </div>
          <div className="rounded-[2rem] border border-gray-200 bg-white p-7 shadow-xl shadow-gray-200/50 sm:p-9">
            <p className="text-lg font-bold leading-8">{t('“Our role is not simply to complete a list of tasks. It is to understand what success looks like, bring the right specialists together, and help our clients make stronger digital decisions.”')}</p>
            <div className="mt-7 border-t border-gray-200 pt-5"><p className="text-sm font-extrabold text-[#bf2227]">{t('The Decasoft approach')}</p><p className="mt-1 text-sm text-gray-500">{t('Customer focused. Quality driven.')}</p></div>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 py-20 sm:px-8 lg:py-28">
        <div className="relative mx-auto max-w-7xl overflow-hidden rounded-[2rem] bg-[#bf2227] px-6 py-12 text-white sm:px-12 lg:flex lg:items-center lg:justify-between lg:px-16 lg:py-16">
          <div className={`absolute -top-24 h-72 w-72 rounded-full border-[40px] border-white/5 ${isRtl ? '-left-16' : '-right-16'}`} />
          <div className="relative max-w-2xl">
            <p className="text-sm font-bold uppercase tracking-[.2em] text-white/70">{t("Let's work together")}</p>
            <h2 className="mt-3 font-montserrat text-3xl font-black leading-tight sm:text-4xl">{t('Looking for a digital partner who sees the bigger picture?')}</h2>
          </div>
          <Link href={href('/contact-us')} className="relative mt-8 inline-flex shrink-0 items-center gap-2 rounded-full bg-white px-7 py-3.5 text-sm font-extrabold text-[#bf2227] transition hover:-translate-y-0.5 lg:ms-10 lg:mt-0">
